...
// Define the structure of the mail data
interface MailData {
  id: number;
  fromName: string;
  fromEmail: string;
...
// Define the structure of the props received by the component
interface Props {
  selectedThread: string;
}

const CenterPage: React.FC<Props> = ({ selectedThread }) => {
  // State to manage the visibility of the reply and delete popups
  const [showPopUp, setShowPopUp] = useState(false);
  const [showDelete, setShowDelete] = useState(false);

  // State to hold the selected mail data
  const [selectedMail, setSelectedMail] = useState<MailData[]>([]);

  // Function to toggle the reply popup
  const togglePopUp = () => {
    setShowPopUp(!showPopUp);
  };

  // Function to handle the deletion of an email thread
  ...
  // Effect to handle keyboard shortcuts for deleting or replying to a mail
  useEffect(() => {
  ...
  // Effect to fetch the selected thread's mail data from the API
  useEffect(() => {
    const fetchMail = async () => {
      if (selectedThread) {
        ...
        // Set default data if no thread is selected
        setSelectedMail([
          {
            id: 0,
            fromName: "",
  ...
  return (
    <div className="overflow-y-scroll no-scrollbar h-full">
      {/* Header section with email details and actions */}
      ...
      {/* Render each mail in the selected thread */}
      <div>
      ...
      {/* Divider with 'View all replies' button */}
      <div className="py-8 mx-8 relative flex justify-center items-center">
...
// Component to display individual mail details
  ...
  return (
    <div className="dark:bg-[#141517] bg-white border dark:border-[#343A40] mx-8 rounded-md my-3">
      <div className="p-4">
        ...
        {/* Display the email body, allowing HTML rendering */}
        <div
